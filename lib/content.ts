...
export async function savePost(post: {
  ...
  author?: string; tags?: string[]; sourceUrl?: string; draftId?: number;
}): Promise<void> {
  await db()`
    ...
    ON CONFLICT (slug) DO UPDATE SET
      ...
      content = EXCLUDED.content, updated_at = now(), tags = EXCLUDED.tags
  `;
}
...
export async function saveDraft(draft: {
  sourceTweetId?: string; sourceAuthor?: string; sourceUrl?: string;
  rawContent: string; topicSummary?: string; suggestedTitle?: string;
}): Promise<Draft | null> {
  const rows = (await db()`
...
export async function updateDraftStatus(id: number, status: DraftStatus, slug?: string, postBody?: string): Promise<void> {
  await db()`
    UPDATE drafts SET status = ${status}, slug = ${slug || null},
      post_body = ${postBody || null}, reviewed_at = now(),
      published_at = ${status === 'approved' ? new Date().toISOString() : null}::timestamptz
    WHERE id = ${id}
  `;
}
...
function mapPost(row: any): Post {
  return {
    ...
    tags: row.tags || [], sourceUrl: row.source_url,
  };
}

function mapDraft(row: any): Draft {
  return {
    id: row.id, sourceTweetId: row.source_tweet_id,
    sourceAuthor: row.source_author, sourceUrl: row.source_url,
    rawContent: row.raw_content, topicSummary: row.topic_summary,
    suggestedTitle: row.suggested_title, status: row.status,
    slug: row.slug, postBody: row.post_body,
